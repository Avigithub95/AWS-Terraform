...
## Minimum two public subnet create use for load balancer and auto scaling in different AZ ###
## Public subnet use for Application server like my Wordpress 
resource "aws_subnet" "pub_sub1" {
  vpc_id                  = aws_vpc.new_vpc.id
  cidr_block              = var.pub_sub1_cidr
  availability_zone       = data.aws_availability_zones.az.names[0] ### name like ap-south-1a is a name not id ##
  map_public_ip_on_launch = var.map_public_ip_on_launch             ## auto assign ip ### True ### instances launched into the subnet should be assigned a public IP address vale is true
  tags = {
